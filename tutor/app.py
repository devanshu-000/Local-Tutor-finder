...
def register():
    ...
    # ── Link subject (case-insensitive dedup, always stored lowercase) ──
    cur.execute("SELECT subject_id FROM subjects WHERE LOWER(subject_name) = LOWER(%s)", (subject,))
    row = cur.fetchone()
    if row:
        subject_id = row['subject_id']
    else:
        cur.execute("INSERT INTO subjects (subject_name) VALUES (%s)", (subject.strip().lower(),))
        mysql.connection.commit()
...
def search():
    ...
    # -------------------------------------------------------------------
    # FIX 1: Accept tutors where EITHER status OR verification_status
    #         is 'approved' — handles legacy rows where only one column
    #         was set. COALESCE guards against NULL verification_status.
    # FIX 2: All string comparisons use LOWER() on both sides so the
    #         search is fully case-insensitive regardless of how data
    #         was stored (e.g. "Maths", "MATHS", "maths" all match).
    # FIX 3: Subject filter uses HAVING (post-GROUP BY) so it correctly
    #         scans the concatenated subject list, preventing rows from
    #         being excluded when a tutor has multiple subjects.
    # -------------------------------------------------------------------
    query = """
        SELECT   t.*,
                 GROUP_CONCAT(s.subject_name ORDER BY s.subject_name SEPARATOR ', ') AS subjects
        ...
        LEFT JOIN subjects       s  ON ts.subject_id = s.subject_id
        WHERE    (
                     LOWER(COALESCE(t.verification_status, '')) = 'approved'
                  OR LOWER(COALESCE(t.status, ''))               = 'approved'
                 )
    """
    values  = []
    having  = []
    ...
    query += " GROUP BY t.tutor_id"

    # Subject filter applied AFTER GROUP_CONCAT so partial matches work
    # across all subjects a tutor teaches (e.g. "math" matches "Mathematics")
    if subject:
        having.append("LOWER(COALESCE(subjects, '')) LIKE %s")
        h_vals.append("%" + subject.lower() + "%")

    if having:
        query += " HAVING " + " AND ".join(having)

    query += " ORDER BY t.tutor_id DESC"

    cur.execute(query, tuple(values + h_vals))
    tutors = cur.fetchall()
